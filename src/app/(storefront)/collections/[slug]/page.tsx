...
import { getCollectionBySlug, listProducts } from "@/lib/data/store";

export default async function CollectionDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const [collection, products] = await Promise.all([getCollectionBySlug(slug), listProducts({ collectionSlug: slug })]);
